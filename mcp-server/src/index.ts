...
server.tool(
  "search_trips",
  "Tra cứu chuyến xe theo điểm đi, điểm đến và ngày đi.",
  {
...
server.tool(
  "get_booking_status",
  "Lấy thông tin vé đã đặt bằng mã vé và email người đặt.",
  {
...
server.tool(
  "get_revenue_summary",
  "Xem thống kê doanh thu theo ngày gần nhất.",
  {},
  async () => {
...
server.tool(
  "get_popular_routes",
  "Lấy top các tuyến đường được tìm kiếm nhiều nhất.",
  {},
  async () => {
...
server.resource(
  ...
  { description: "Chính sách hủy vé và hoàn tiền" },
  async (uri) => {
    const policy = `
...
server.resource(
  "policy-checkin",
  "bus://policy/checkin",
  { description: "Hướng dẫn thủ tục lên xe (Check-in)" },
  async (uri) => {
    const checkin = `
